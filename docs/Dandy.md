...
while True:
    ...
    time.sleep(1)
``` 
#### 3.7.1 Option B: Circuitpython and the RPiPico

#### 3.7.1 Option C: Micropython and the CY8CProto

#### 3.7.1 Option D: Arduino

#### 3.7.2 Writing without the GUI
Here's my code... Clean up the part with setting port.
...
#port='/dev/ttyACM0'
baudrate=115200
serialPort=serial.Serial(port=port, baudrate=baudrate, bytesize=8, timeout=0.1, \
...
#### 3.7.3 Writing with GUI
Note that we don't need the asyncio trick here. We only have one loop, for the GUI.
```python
import asyncio
import tkinter as tk
import time
...
### 4.1 DANDY widgets for analog inputs, no HW

#### 4.1.1 A first example, displaying one value
...
#### 4.1.2 A second example, displaying xyz values.
Here's my example
```python
